...
## Design-refinement references

Candidate texture/whimsy demos gathered 2026-07-06, reviewed + annotated the same day (fit verdict against the locked hard-square / hard-offset-shadow / `feTurbulence`-grain direction below). Reimplement from scratch rather than copy-paste (see licensing note below); treat these as inspiration only.
