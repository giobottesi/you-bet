...
| 11 | Licensing / OSS | `LICENSE` added (AGPL-3.0); dependency licenses still unaudited and no `CONTRIBUTING`/`NOTICE` | Third-party dependency licenses are unaudited for AGPL-compatibility; no `CONTRIBUTING`/`NOTICE` for outside contributors | Audit dependency-license compatibility, add attribution/`NOTICE` + `CONTRIBUTING` if opening to contributors | 🟢 | session 2026-07-12 |
