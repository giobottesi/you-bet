...
| 11 | Licensing / OSS | No `LICENSE` file and open-source compliance not established | Public competition repo with no license leaves reuse rights ambiguous; third-party dependency licenses are unaudited; no `CONTRIBUTING`/`NOTICE` for outside contributors | Choose and add a `LICENSE`, audit dependency-license compatibility, add attribution/`NOTICE` + `CONTRIBUTING` if opening to contributors | 🟡 | session 2026-07-12 |
