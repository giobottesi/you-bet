...
| 6 | Copy / i18n | Bet-type card "house edge" term is jargon; badge shows `casa X%` + a native `title` tooltip as a stopgap | "Edge" isn't understood by BR bettors; the picker mixes hardcoded-English headings with i18n'd type names | Localize the whole picker (heading/hint/badge) and design a proper house-edge explainer (inline glossary or first-use hint), not a raw `title` | 🟡 | FE 02 review |
