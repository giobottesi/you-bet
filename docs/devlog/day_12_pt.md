...
A divisão da validação caiu da própria textura da plataforma: um grupo de radio já significa "escolha exatamente um", então o `required` nativo no grupo compartilhado `weekly_amount_cents` carrega essa restrição de graça — onde o caso de ≥1-checkbox do FE-02 teve que fazer isso na mão com `setCustomValidity`. O controller Stimulus só cuida do que o HTML não faz: digitar no campo custom seleciona o radio dele, sincroniza reais→cents, e — o único ponto em que o FE-03 ainda recorre ao `setCustomValidity` — bloqueia o submit até esse valor custom passar de zero. Specs de helper cobrem o mapa de âncoras e o label `R$12`; specs de request checam um radio required por âncora mais a linha custom. O form ainda está desligado (`url: "#"`) — ligar `weekly_amount_cents` a um atributo persistido é um card de BE depois.
