...
1. **Source / fact** — every concrete claim (number, PR ref, route, model/method name, date, house edge) is true against the code or a cited primary source. Deep-link citations, don't homepage them. Verify, never trust prose or memory. This is the highest-priority check: the worst failure is copy that contradicts the code (a doc once said the pre-commit hook "skips when the DB is down" after it was changed to require the DB).
   - **A citation states the source's own printed number — never our math.** The "exact / receipt" layer must be checkable *verbatim* against the linked page: a reader lands on the source and sees that exact figure. Do NOT compute, invert, or convert in the citation — no "100−58=42%", no "16% × população = 22,1 mi", no "0,2255 ÷ 0,0709 ≈ 3x". If the source prints 16% and 58%, the citation says 16% and 58%. Derived/rounded numbers may live in the *plain skim* line (clearly the readable version), but the receipt is raw source truth, and plain and receipt must not contradict. If a number can't be found verbatim in the linked source, either re-link to the source that does print it, or drop it — an unverifiable citation on a "verified sources" page is the worst failure. Also: never truncate a source's figure set (e.g. cite two of three regression coefficients) — state the full set the source gives.
...
4. **Tone** (competition rule, `docs/PROPOSAL.md`) — target the industry and its math; **never** blame, ridicule, or judge the bettor. Wit punches up (house, odds, predatory design), never down. Warm, knowing, anti-Tigrinho.
   - **Don't over-correct into fatalism.** Punching up at the industry ≠ removing the reader's agency. A *conclusion/takeaway* line stays **factual and sober** (what the data shows), not a marketed slogan or rallying cry — a slogan that pins everything on "the system" can read as inducing passivity ("nothing I can do"). State the finding plainly; let the reader act. "O país já enxergou o problema e quer freio" (slogan) → "A maioria dos brasileiros considera as bets prejudiciais e quer mais regras" (factual). Reserve editorial punch for framing *how harm happens*; keep the takeaway a fact.
...
- **2026-07-12 (fidelity under reprocessing, from Gio's review)** — Copy that's been reprocessed many times (translate → plain-pass → reframe → round) drifts numerically even when each step looks safe. Three drifts slipped through here, all in the *rounded plain* line while the exact receipt stayed right: "3 vezes mais que / 3 times more than" (reads 4x; true ~3.18x → "cerca de 3x / 3 times as much"), "Metade/Half" for 52% (understates — 52% is *more* than half), "7 em cada 10 / 7 in 10" for 73% (understates — 73% ≈ 3/4). **Rules:** (1) rounding may lose precision but must **NEVER flip the inequality** — 52%→"half" drops below, 73%→"7 in 10" drops, "3x mais" inflates up; hedge in the true direction ("mais da metade", keep "73%", "cerca de 3x"). (2) **Pair every rounded plain claim with an `exact` receipt** carrying the precise figure + source; the receipt is the fidelity anchor and the plain line may not contradict it. (3) **Vary the comparison device** — don't render every stat as "N em cada M"; it reads formulaic (AI tell) and Gio flags it. Mix %, "mais da metade", one clean fraction ("2 em cada 3" = 66,7% is exact). (4) **No stale relative time** on public copy — "no último mês" goes stale + is ambiguous against a 2024 source; anchor it ("em 2024", "em 30 dias"). (5) After heavy reprocessing, **re-audit every number against `docs/DATA.md` before shipping** — treat the source-of-truth table as the gate, not the prose.
